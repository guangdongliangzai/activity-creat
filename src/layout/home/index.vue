...
//   files[key]().then((res) => {
//     modules[res.default.name] = res.default;
//   });
// }
...
requireComponent.keys().forEach((fileName) => {
  ...
  // 全局注册组件
  modules[componentConfig.default.name] = componentConfig.default || componentConfig;
});
...
export default {
  name: "home",
  components: { vuedraggable, ...modules }, //, ...componentsFiles
  setup() {
    // 是否显示预览
      ...
      if (datas.pageComponents.length) {
        ...
        for (let i = 0, l = childrens.length; i < l; i++) {
          ...
          if (childoffset + childrens[i].clientHeight / 2 > event.offsetY) {
            ...
            if (!datas.pageComponents[i + 1] || datas.pageComponents[i + 1].type === 0)
              break;
